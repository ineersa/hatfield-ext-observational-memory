...
/**
 * Normative REFLECTOR_SYSTEM prompt — faithful Pi port.
 *
 * Hatfield adaptations only: supporting_observation_ids field name, privacy line,
 * and no dropper-coverage language that would invent complete-generation duties.
 */
...
- A good reflection captures meaning that should survive after individual observations leave active compacted memory.
...
- Every reflection must include supporting_observation_ids from the current observations list.
...
- supporting_observation_ids are a coverage/provenance set: include all current observation ids whose durable meaning is preserved by the reflection with equivalent fidelity and can later be treated as redundant active-memory detail.
- supporting_observation_ids are not a checklist to cover every observation. Do not add ids merely to improve coverage counts, maximize support ids, or maximize strong coverage.
- False or inflated support ids can cause unsafe downstream pruning, including removal of high-resistance active observations whose meaning was not actually preserved.
...
- Never invent observation ids. Proposals with missing, empty, or invalid supporting_observation_ids are rejected.
